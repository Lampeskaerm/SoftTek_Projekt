...
public class Screen extends JPanel implements Runnable {
	...
	public void movementGhosts(){
		...
		redGhost = (int)(Math.random()*4 + 1);
		pinkGhost = (int)(Math.random()*4 + 1);
		cyanGhost = (int)(Math.random()*4 + 1);
		orangeGhost = (int)(Math.random()*4 + 1);
		
		if(redGhost == 1){
			getLocationRedGhost();
			if(Array[locationRedGhostx+1][locationRedGhosty] == 1){
				if(locationRedGhostx < Width - 1){
					Array[locationRedGhostx + 1][locationRedGhosty] = 3;
					Array[locationRedGhostx][locationRedGhosty] = 1;
				}
			}
			if(Array[locationRedGhostx+1][locationRedGhosty] == 0){
				if(locationRedGhostx < Width - 1){
					Array[locationRedGhostx + 1][locationRedGhosty] = 3;
					Array[locationRedGhostx][locationRedGhosty] = 0;
				}
			}
		}
		...
		if(redGhost == 3){
			getLocationRedGhost();
			if(Array[locationRedGhostx][locationRedGhosty+1] == 1){
				if(locationRedGhosty < Heigth - 1){
					Array[locationRedGhostx][locationRedGhosty+1] = 3;
					Array[locationRedGhostx][locationRedGhosty] = 1;
				}
			}
			if(Array[locationRedGhostx][locationRedGhosty+1] == 0){
				if(locationRedGhosty < Heigth - 1){
					Array[locationRedGhostx][locationRedGhosty+1] = 3;
					Array[locationRedGhostx][locationRedGhosty] = 0;
				}
		...
		try {
			Thread.sleep(1000); //stops the program for 20 milliseconds before going on, therefore the max fps is 50 fps.
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
	}
